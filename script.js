...
const angkaValid = (...vals) => vals.every(v => v !== '' && v !== null && !isNaN(v));
...
function hitungBarisanAritmetika() {
    ...
    if (!angkaValid(a, b, n) || n <= 0) {
        showResult('resultBarisanAritmetika', `
    ...
    const deretText = suku.map(v => formatNumber(v)).join(', ') + (n > tampilMaks ? ', ...' : '');
...
function hitungBarisanGeometri() {
    ...
    if (!angkaValid(a, r, n) || n <= 0) {
        showResult('resultBarisanGeometri', `
            <div class="warning-box" style="padding: 10px; border-radius: 6px; background: #fef2f2; border-left: 3px solid #ef4444; color: #991b1b;">
...
function hitungDeretAritmetika() {
    ...
    if (!angkaValid(a, b, n) || n <= 0) {
        showResult('resultDeretAritmetika', `
            <div class="warning-box" style="padding: 10px; border-radius: 6px; background: #fef2f2; border-left: 3px solid #ef4444; color: #991b1b;">
                <strong>⚠️ Input tidak valid!</strong> Pastikan a, b, dan n (n > 0) terisi angka.
            </div>`);
        return;
...
function hitungDeretGeometri() {
    ...
    if (!angkaValid(a, r, n) || n <= 0) {
        showResult('resultDeretGeometri', `
            <div class="warning-box" style="padding: 10px; border-radius: 6px; background: #fef2f2; border-left: 3px solid #ef4444; color: #991b1b;">
                <strong>⚠️ Input tidak valid!</strong> Pastikan a, r, dan n (n > 0) terisi angka.
            </div>`);
        return;
...
function checkAnswer(btn) {
    ...
    if (input.value.trim() === '' || isNaN(user)) {
        ...
        feedback.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Masukkan jawaban terlebih dahulu!';
        return;
    }
    ...
    if (isCorrect) {
        ...
        feedback.innerHTML = '<i class="fas fa-check-circle"></i> Jawaban Benar! 🎉';
        ...
        feedback.className = 'feedback show wrong';
        feedback.innerHTML = '<i class="fas fa-times-circle"></i> Jawaban Belum Tepat. Coba lagi!';
